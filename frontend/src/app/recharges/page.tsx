...
export default function RechargesIndexPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`${API_BASE}/games?active=1&context=recharge&per_page=200`, {
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2 text-xs text-white/60">
            ...
            <span className="text-white/80">Recharges</span>
          </div>
          <h1 className="text-2xl font-black tracking-tight sm:text-3xl">Recharges</h1>
          <p className="text-sm text-white/60">Choisis ton jeu pour afficher les recharges disponibles.</p>
        </div>
          ...
          {loading
            ...
            : filtered.map((g) => {
                ...
                return (
                  <Link
                    key={g.id}
                    href={`/recharges/${encodeURIComponent(String(g.slug))}`}
                  ...
                  >
                    ...
                    <div className="mt-4">
                      ...
                      <p className="mt-1 text-sm text-white/60">Voir les recharges</p>
                    </div>
                  </Link>
                );
              })}
